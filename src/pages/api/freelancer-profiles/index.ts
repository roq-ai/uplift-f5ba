...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getFreelancerProfiles() {
    ...
    const response = await prisma.freelancer_profile
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'freelancer_profile'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createFreelancerProfile() {
    await freelancerProfileValidationSchema.validate(req.body);
    const body = { ...req.body };
    ...
    const data = await prisma.freelancer_profile.create({
      data: body,
    });
